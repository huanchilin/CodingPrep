...
// we can use two stacks to sort. first put the top input stack to out, then we store the next top
// at input, we dump all output stack nodes that are smaller or equal to input then push the cached
// to output. then we return all values from input to output that are smaller. To make sure everything
// is in order, we update the entity values
