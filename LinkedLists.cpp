...
// important that we use recursive and make sure we check the nullptrs
...
bool isLinkListPalindrome(LinkNode<int>* a) {
	if (a == nullptr) return false;
	if (a->next == nullptr) return true;

	int step = 0; 
	int accumulate = 0;
	LinkNode<int>* oneStep = a;
	LinkNode<int>* twoStep = a;

	while (twoStep->next && twoStep->next->next) {
		step++;
		oneStep = oneStep->next;
		accumulate += oneStep->data;
		twoStep = twoStep->next->next;
	}
	int totalLength = step * 2;
	if (twoStep->next) totalLength++;
...
LinkNode<int>* intersectNode(LinkNode<int>* a, LinkNode<int>* b) {
	if (!a || !b) return nullptr;

	unordered_set<LinkNode<int>*> set;
	auto node = a;
...
// another way to implement this is to have one fast and one slow pointers.
// when both collide, they are at the same node.
LinkNode<int>* loopDetection(LinkNode<int>* a) {
	if (!a) return nullptr;
